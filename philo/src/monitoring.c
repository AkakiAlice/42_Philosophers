...
/*   Updated: 2022/12/10 14:29:22 by alida-si         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*monitoring(void *arg)
{
	...
	{
		usleep(200);
		if (read_var(&aux->nb_eat, &aux->mutex_nb_eat) == aux->rules->must_eat && &aux->rules->must_eat != 0)
		{
			aux->satisfied = 1;
		}
		...
		if ((time_diff - read_var(&aux->last_meal, &aux->mutex_last_meal)) >= read_var(&aux->rules->time_to_die, &aux->data->mutex_var))
		{
